...
def main():
    ...
    CURRENT_THROED = None
    while True:
        print_line2(f'Agent Looping ... AUTO_MANAGER_ID={auto_manager_id}, PID={pid}')
        ...
        run_py_fullpath = agent_env.BASE_DIR + run_py_file
        
        found_proc_count = find_process_count(run_py_file, agent_env.SERVER_ID, agent_env.AGENT_ID, agent_env.GROUP_ID)
        logger.info(f'    현재상태 : 진행상태=[{status_cd}], 자동화단계=[{au_x}], 프로세스 개수=[{found_proc_count}]')
        ...
        print_line1(f'CURRENT AGENT : AGENT_ID=[{auto_manager_id}], STATUS_CD=[{status_cd}]')
        ...
        time.sleep(agent_env.LOOP_WAIT_AGENT)
