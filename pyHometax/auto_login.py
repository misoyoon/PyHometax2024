...
# 위택스 인증서 로그인
def login_wetax(driver:WebDriver):
    driver.get("https://www.wetax.go.kr/loginMbr.do")
    logt("로그인 페이지 이동 대기", 2)
    
    driver.find_element(By.CSS_SELECTOR, '#btnCertLogin').click()
    logt("패스워드 입력 창 대기", 2)
    
    pyautogui.press('tab', presses=5, interval=0.1) # enter 키를 3초에 한번씩 세번 입력합니다. 
    pyautogui.press('down')
    pyautogui.press('tab', presses=4, interval=0.1) # enter 키를 3초에 한번씩 세번 입력합니다. 
    pyautogui.write('kksjns1203!', interval=0.1)
    pyautogui.press('tab', presses=2, interval=0.1)
    pyautogui.press('enter')
    logt("로그인 완료 후 메인페이지 이동", 1)    
...
def login_hometax(driver, login_info, IS_DEBUG):
    if IS_DEBUG:
        # 쿠키 로그인
        TXPP = 'QsnxvhNE1LTaaRpDvm3yFTCEnOjxZMQGo8EMeUq1XlqqiD2rx1bY4E1D5UIzGwD2.tupiwsp18_servlet_TXPP01'
        TEHT = 'feEkpBb5Vvald1qLv7aqPj9IJ9H9UVX2oYQtgnAEIhwP2TolxFOTDmKRp2YcdgH4.tupiwsp06_servlet_TEHT04'
        login_hometax_use_cookie(driver, TXPP, TEHT)
    else:
