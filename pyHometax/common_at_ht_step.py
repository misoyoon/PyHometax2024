...
if len(sys.argv) < 4:
    print("실행 Parameter가 정상적이지 않습니다.")
    exit()
...
def init_step_job():
    ...
    logt("###########################################")
    logt("서버정보: auto_manager_id=%s,  WORKER=%s(%s), AUX=%s" % (auto_manager_id, worker_nm, worker_id, au_x))
    logt("###########################################")

    if not (status_cd == 'RW' or status_cd == 'R'):
        logt(f"현재 상태 : status cd = {status_cd}  ==> 작업 중료")
        exit()
    ...
    if au_x == '1' or au_x == '2' or au_x == '5':
        ...
        # input 태그 찾기
        try:
            #driver.find_element(By.CSS_SELECTOR, '#chk_today_close1').click()
